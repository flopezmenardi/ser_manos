...
class ProfilePictureButton extends ConsumerWidget {
  ...
  const ProfilePictureButton({super.key, required this.text, this.onPressed});
...
Future<void> _takeProfilePhoto(BuildContext context, WidgetRef ref) async {
...
Future<void> _handlePhoto(WidgetRef ref, XFile photo) async {
  ...
  try {
    await ref.read(profileControllerProvider).uploadProfilePicture(uid, photo); // already on your interface
